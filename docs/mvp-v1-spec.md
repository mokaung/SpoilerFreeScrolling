...
- **Each row:** **Topic name** (profile title), **progress** (formatted by media type—see **Progress display by media type**; Book: “Ch 5”, TV: “S3 E5”, Movie: omit progress entirely, Game: truncated freeform), **chevron** (right) to indicate clickable, **delete** button (e.g. icon). Row (and chevron) navigate to Profile Detail. Delete opens Delete confirmation modal; on confirm, remove profile and stay on list.
...
- **Actions:** “New” button → Create Profile Page. No edit from list; edit only from Detail.
...
- **Actions (view mode):**
  - **Regenerate keywords** – Call LLM with title, media type, `userProgress`, and optional extra context; LLM returns strict JSON; parse into `KeyWordRule[]`, save, then refresh. Show LoadingSpinner during request; on success, update displayed keywords.
...
- **Profile list:** Rendered as rows. Each row shows topic name, **progress (formatted by media type)**, chevron, delete. `ProfileListRow` uses shadcn Button for delete and row click/chevron; progress display follows **Progress display by media type**.
- **ProfileDetailView:** Read-only allowed accounts as a list; in edit mode uses `AllowedAccountsInput`. Progress and keywords behavior follow **Data & behavior** (progress by media type, LLM JSON for keywords).
